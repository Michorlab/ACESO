---
title: "Complex dosing schedules"
...
1 compartment model, no depot, parameters: CL and V

```{r}
code_iiv <- '$PARAM @annotated
TVCL   :  171.36 : Clearance (volume/time)
TVV    : 153.547 : Central volume (volume)
...
Read the model:
```{r}
 cmt1_iv<- mcode("mymodel", code_iiv)%>% Req(CP) 
```
...
## Oral administration
Now we are going to simulate a one compartment model with extravascular administration. In this case, we are going to select a model from the model libraries already included in ACESO instead of writing all the code by ourselves:
```{r}
